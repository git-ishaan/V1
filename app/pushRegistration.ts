  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // 3️⃣ Expo token - UPDATED: with explicit project ID and error handling
        let token;
        try {
          const { data } = await Notifications.getExpoPushTokenAsync({
            projectId: '5d65cd19-8356-4c54-b595-c8c4954092f7',
          });
          token = data;
        } catch (tokenError) {
          console.warn('[PushRegistration] Failed with project ID, trying without:', tokenError);
